...
def get_vm_ctpy_all_history (
    ...
    """
    Read all counterparty VM history available up to the requested date.
    """
...
def get_vm_ice_all_history (
    ...
    """
    Read all ICE VM history available up to the requested date.
    """
    date = str_to_date(date)
    fund = AEGIS_DISC_FUND_HV if fund is None else fund
...
def get_vm_ctpy_values_by_date (
    ...
    """
    Return counterparty VM rows for the latest available date.
    """
    date = str_to_date(date)
    fund = AEGIS_DISC_FUND_HV if fund is None else fund

    dataframe, md5 = get_vm_ctpy_all_history(date, fund) if dataframe is None else (dataframe, md5)
    schema_overrides = AEGIS_IM_CTPY_COLUMNS if schema_overrides is None else schema_overrides
...
def get_vm_ice_values_by_date (
    ...
    """
    Return ICE VM rows for the latest available date.
    """
    date = str_to_date(date)
    fund = AEGIS_DISC_FUND_HV if fund is None else fund

    dataframe, md5 = get_vm_ice_all_history(date, fund) if dataframe is None else (dataframe, md5)
    schema_overrides = AEGIS_IM_ICE_COLUMNS if schema_overrides is None else schema_overrides
